...
import { CREATE_USER_API, GET_USER_DETAILS, LOGIN_USER_API, GET_DOCUMENTS, CREATE_DOCUMENT_API } from './API';
...
export const getDocuments = (parentDocumentId: string): Promise<Document[]> => {
    return new Promise((resolve, reject) => {
        ...
        axios.get(url, config).then((response) => resolve(response.data)).catch((err) => reject(err))
    });
}
